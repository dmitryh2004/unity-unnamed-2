...
public class PlayerInteractor : MonoBehaviour
{
    ...
    private void UpdateInteractionHints()
    {
            ...
            if (hit.collider.TryGetComponent(out Interactable interactable))
            {
                ...
                if (hit.distance < interactable.GetInteractionRange())
                {
                    if (interactable is DoorController doorController)
                    {
                        ...
                        else if (doorController.CheckTriggerZone && doorController.InTriggerZone)
                        {
                            doorObstacleHintActive = true;
                        }
                        else
                        {
                            if (doorController.IsOpen())
                    ...
                    else if (interactable is SpaceshipPanelController spaceshipPanelController)
                    {
        ...
        HintManager.Instance.ActivateHint(HintManager.Instance.GetHintByName("DoorObstacleHint"), doorObstacleHintActive);
    }
}
